...
#import "UIWindow+MSVisibleTouches.h"
#import "NSObject+MSMethodSwizzling.h"

#define TOUCH_WIDTH_HEIGHT 40
...
//    return [self initWithFrame:CGRectMake(0, 0, 50, 50)];
    return [self initWithFrame:CGRectMake(0, 0, TOUCH_WIDTH_HEIGHT, TOUCH_WIDTH_HEIGHT)];
}
...
@implementation UIWindow (MSVisibleTouches)
...
+ (void)load {
    ...
    dispatch_once(&onceToken, ^{
        [UIWindow ms_swizzleInstanceMethod:@selector(sendEvent:) with:@selector(ms_sendEvent:)];
        [UIWindow ms_swizzleInstanceMethod:@selector(layoutSubviews) with:@selector(ms_layoutSubviews)];
    });
}

#pragma mark - Propertys

- (BOOL)ms_touchesVisible {
...
- (void)setMs_touchesVisible:(BOOL)touchesVisible {
...
- (void)ms_sendEvent:(UIEvent *)event {
    //  获取所有的触摸对象
